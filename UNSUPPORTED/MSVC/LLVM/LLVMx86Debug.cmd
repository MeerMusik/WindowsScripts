...
set LINK=/NOLOGO /CGTHREADS:%NUMBER_OF_PROCESSORS% /DYNAMICBASE /LARGEADDRESSAWARE /NXCOMPAT
set PATH=%PATH%;%SystemDrive%\Dev\CMake\bin\;%SystemDrive%\Dev\Ninja\;%SystemDrive%\Dev\Pythonx86\;%SystemDrive%\Dev\gnuwin32\bin\;%SystemDrive%\Dev\Perlx86\perl\bin\;%SystemDrive%\Dev\Git\bin\
if not exist %SystemDrive%\Dev\LLVMx86BuildDebug\ ( 
mkdir %SystemDrive%\Dev\LLVMx86BuildDebug\ 
goto :BUILD 
)

if exist %SystemDrive%\Dev\LLVMx86BuildDebug\ ( 
cd %SystemDrive%\Dev\LLVMx86BuildDebug\
goto :CLEANDIRECTORY 
)
...
del *.* /s /f /q
cd %SystemDrive%\
rmdir %SystemDrive%\Dev\LLVMx86BuildDebug\ /s /q
...
cmake %SystemDrive%\Dev\GitRepos\llvm-project\llvm\ -G"Ninja" -DCMAKE_VERBOSE_MAKEFILE=ON -DLLVM_ENABLE_PROJECTS="clang;clang-tools-extra;compiler-rt;libcxx;libcxxabi;lld;lldb;llvm" -DCMAKE_INSTALL_PREFIX=%SystemDrive%\Dev\LLVMx86Debug\ -DCMAKE_BUILD_TYPE="Debug" -DLLVM_TARGETS_TO_BUILD="X86" -DLLVM_INCLUDE_EXAMPLES="OFF" -DLLVM_INCLUDE_TESTS="OFF" -DLLVM_INCLUDE_DOCS="OFF" -DLLVM_PARALLEL_COMPILE_JOBS="%NUMBER_OF_PROCESSORS%" -DLLVM_PARALLEL_LINK_JOBS="%NUMBER_OF_PROCESSORS%" -DLLVM_ENABLE_EH="ON" -DLLVM_ENABLE_RTTI="ON" -DPYTHON_HOME=%SystemDrive%\Dev\Pythonx86\ -DSWIG_DIR=%SystemDrive%\Dev\swigwin\ -DSWIG_EXECUTABLE=%SystemDrive%\Dev\swigwin\swig.exe && ^
rem Default Flags Part 2 of 2:
ninja && ^
rem Enable Debug Logging Part 1 of 2:
rem cmake --verbose --debug-output --debug-find --log-level=TRACE --trace --trace-expand --trace-format=human --trace-redirect="%SystemDrive%\Dev\CMakeTraceLog_LLVMx86Debug_Configure_%DATE:/=-%.log" %SystemDrive%\Dev\GitRepos\llvm-project\llvm\ -G"Ninja" -DCMAKE_VERBOSE_MAKEFILE=ON -DLLVM_ENABLE_PROJECTS="clang;clang-tools-extra;compiler-rt;libcxx;libcxxabi;lld;lldb;llvm" -DCMAKE_INSTALL_PREFIX=%SystemDrive%\Dev\LLVMx86Debug\ -DCMAKE_BUILD_TYPE="Debug" -DLLVM_TARGETS_TO_BUILD="X86" -DLLVM_INCLUDE_EXAMPLES="OFF" -DLLVM_INCLUDE_TESTS="OFF" -DLLVM_INCLUDE_DOCS="OFF" -DLLVM_PARALLEL_COMPILE_JOBS="%NUMBER_OF_PROCESSORS%" -DLLVM_PARALLEL_LINK_JOBS="%NUMBER_OF_PROCESSORS%" -DLLVM_ENABLE_EH="ON" -DLLVM_ENABLE_RTTI="ON" -DPYTHON_HOME=%SystemDrive%\Dev\Pythonx86\ -DSWIG_DIR=%SystemDrive%\Dev\swigwin\ -DSWIG_EXECUTABLE=%SystemDrive%\Dev\swigwin\swig.exe && ^
rem Enable Debug Logging Part 2 of 2:
rem ninja -v 1>%SystemDrive%\Dev\LLVMx86Debug_Build_%DATE:/=-%.log 2>&1 && ^
echo DISABLED: pause; && ^
cd %SystemDrive%\Dev\LLVMx86BuildDebug\ && ^
