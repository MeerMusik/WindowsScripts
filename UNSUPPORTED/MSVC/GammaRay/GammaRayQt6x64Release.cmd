...
set PATH=%PATH%;%SystemDrive%\Dev\CMake\bin\;%SystemDrive%\Dev\Qt6x64MSVC\bin\;%SystemDrive%\Dev\Ninja\
if not exist %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\ ( 
mkdir %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\
goto :CHECKFOREXISTINGBUILDDIR 
)

if exist %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\ ( 
cd %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\
...
if not exist %SystemDrive%\Dev\GammaRayQt6Bldx64MSVC\ ( 
mkdir %SystemDrive%\Dev\GammaRayQt6Bldx64MSVC\
goto :BUILD 
)

if exist %SystemDrive%\Dev\GammaRayQt6Bldx64MSVC\ ( 
cd %SystemDrive%\Dev\GammaRayQt6Bldx64MSVC\
...
:BUILD
cd %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\ && ^
robocopy /s /NJS /NDL /NJH /nc /ns /np %SystemDrive%\Dev\GitRepos\GammaRay\ %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\ /xd ".git" & ^
cd %SystemDrive%\Dev\GammaRayQt6Bldx64MSVC\ && ^
cmake -G "Ninja" -DCMAKE_INSTALL_PREFIX=%SystemDrive%\Dev\GammaRayQt6x64MSVC\ %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\ && ^
ninja && ^
ninja install && ^
cd %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\ && ^
del *.* /s /f /q && ^
...
timeout /t 8 /nobreak && ^
rmdir %SystemDrive%\Dev\GammaRayQt6Srcx64MSVC\ /s /q && ^
rmdir %SystemDrive%\Dev\GammaRayQt6Bldx64MSVC\ /s /q && ^
echo DONE! && ^
pause;
