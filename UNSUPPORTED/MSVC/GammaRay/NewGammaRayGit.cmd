...
title CREATE LOCAL GAMMARAY BRANCH FROM EXTERNAL GIT BRANCH
:START
set PATH=%PATH%;%SystemDrive%\Dev\Git\bin\
if not exist %SystemDrive%\Dev\GitRepos\ ( 
mkdir %SystemDrive%\Dev\GitRepos\
goto :CHECKOUTGAMMARAY 
)

if exist %SystemDrive%\Dev\GitRepos\ ( 
cd %SystemDrive%\Dev\GitRepos\
goto :CHECKFORGAMMARAYDIR 
)

:CHECKFORGAMMARAYDIR
if exist %SystemDrive%\Dev\GitRepos\GammaRay\ ( 
cd %SystemDrive%\Dev\GitRepos\GammaRay\
goto :CLEANDIRECTORY 
)

if not exist %SystemDrive%\Dev\GitRepos\GammaRay\ ( 
cd %SystemDrive%\Dev\GitRepos\
goto :CHECKOUTGAMMARAY 
)
...
timeout /t 4 /nobreak
rmdir %SystemDrive%\Dev\GitRepos\GammaRay\ /s /q
