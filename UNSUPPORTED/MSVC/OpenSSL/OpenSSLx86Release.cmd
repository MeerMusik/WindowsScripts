...
title BUILD OPENSSL-X86 RELEASE DYNAMIC MSVC
:START
...
set LINK=/NOLOGO /CGTHREADS:8 /DYNAMICBASE /LTCG:STATUS /NXCOMPAT
set PATH=%PATH%;%SystemDrive%\Dev\NASMx86\;%SystemDrive%\Dev\Perlx86\perl\bin\;%SystemDrive%\Dev\JOM\
if not exist %SystemDrive%\Dev\OpenSSLSrcx86MSVC\ ( 
mkdir %SystemDrive%\Dev\OpenSSLSrcx86MSVC\
goto :BUILD 
)

if exist %SystemDrive%\Dev\OpenSSLSrcx86MSVC\ ( 
cd %SystemDrive%\Dev\OpenSSLSrcx86MSVC\
goto :CLEANDIRECTORY 
)
...
del *.* /s /f /q
cd %SystemDrive%\
rmdir %SystemDrive%\Dev\OpenSSLSrcx86MSVC\ /s /q
...
robocopy /s /NJS /NDL /NJH /nc /ns /np %SystemDrive%\Dev\GitRepos\openssl\ %SystemDrive%\Dev\OpenSSLSrcx86MSVC\ /xd ".git" ".github" "boringssl" "demos" "doc" ".jenkins" "krb5" "pyca-cryptography" ".travis" "vms" & ^
perl Configure shared --release no-idea no-mdc2 no-rc5 no-comp no-deprecated no-weak-ssl-ciphers no-ssl3 no-ssl3-method no-tls1 no-tls1-method no-tls1_1 no-tls1_1-method no-dtls1 no-dtls1-method VC-WIN32 --prefix=%SystemDrive%\Dev\OpenSSLx86MSVC\ --openssldir=%SystemDrive%\Dev\OpenSSLx86MSVC\ && ^
