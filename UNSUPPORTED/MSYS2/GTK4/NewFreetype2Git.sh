...
if [ -d ${SYSTEMDRIVE}/Dev/GitRepos/freetype/ ]; then rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/GitRepos/freetype/*.* ${SYSTEMDRIVE}/Dev/GitRepos/freetype/* ${SYSTEMDRIVE}/Dev/GitRepos/freetype/
fi
cd ${SYSTEMDRIVE}/Dev/GitRepos/
git clone https://gitlab.freedesktop.org/freetype/freetype.git --verbose --progress
echo DONE!
exit
