...
if [ -d ${SYSTEMDRIVE}/Dev/GitRepos/gobject-introspection/ ]; then rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/GitRepos/gobject-introspection/*.* ${SYSTEMDRIVE}/Dev/GitRepos/gobject-introspection/* ${SYSTEMDRIVE}/Dev/GitRepos/gobject-introspection/
fi
cd ${SYSTEMDRIVE}/Dev/GitRepos/
git clone https://gitlab.gnome.org/GNOME/gobject-introspection.git --verbose --progress
echo DONE!
exit
