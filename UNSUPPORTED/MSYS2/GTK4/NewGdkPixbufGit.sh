...
if [ -d ${SYSTEMDRIVE}/Dev/GitRepos/gdk-pixbuf/ ]; then rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/GitRepos/gdk-pixbuf/*.* ${SYSTEMDRIVE}/Dev/GitRepos/gdk-pixbuf/* ${SYSTEMDRIVE}/Dev/GitRepos/gdk-pixbuf/
fi
cd ${SYSTEMDRIVE}/Dev/GitRepos/
git clone https://gitlab.gnome.org/GNOME/gdk-pixbuf.git --verbose --progress
echo DONE!
exit
