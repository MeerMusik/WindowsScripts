...
if [ -d ${SYSTEMDRIVE}/Dev/GitRepos/glibmm/ ]; then rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/GitRepos/glibmm/*.* ${SYSTEMDRIVE}/Dev/GitRepos/glibmm/* ${SYSTEMDRIVE}/Dev/GitRepos/glibmm/
fi
cd ${SYSTEMDRIVE}/Dev/GitRepos/
git clone https://gitlab.gnome.org/GNOME/glibmm.git --verbose --progress
echo DONE!
exit
