...
if [ -d ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/ ]; then rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/*.* ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/* ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/
fi
if [ ! -d ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/ ]; then cp --force --recursive --verbose ${SYSTEMDRIVE}/Dev/GitRepos/openssl/ ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/
fi
cd ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/
perl Configure CFLAGS='-D_FORTIFY_SOURCE=2 -D_GLIBCXX_ASSERTIONS -fasynchronous-unwind-tables -fexceptions -fcf-protection=branch -finput-charset=utf-8 -fno-lto -fno-omit-frame-pointer -fPIC -fstack-protector-all -grecord-gcc-switches -mfunction-return=thunk -mindirect-branch=thunk -mindirect-branch-register -pthread -Os -g1 -pipe -Wall -Walloc-zero -Wcast-align -Wconversion -Wdouble-promotion -Wduplicated-branches -Wduplicated-cond -Wextra -Wformat=2 -Wformat-nonliteral -Wformat-overflow=2 -Wformat-security -Wformat-truncation -Wlogical-op -Wmisleading-indentation -Wmissing-declarations -Wnonnull -Wnull-dereference -Wpedantic -Wpointer-arith -Wshadow -Wsign-conversion -Wstack-protector -Wstrict-overflow -Wtrampolines -Wvla -Wl,-Os -Wl,--allow-multiple-definition -Wl,--build-id=sha1 -Wl,--default-image-base-high -Wl,--discard-locals -Wl,--dynamicbase -Wl,--enable-auto-image-base -Wl,-fno-lto -Wl,--high-entropy-va -Wl,--nxcompat -Wl,-Bdynamic' shared --release no-idea no-mdc2 no-rc5 no-comp no-deprecated no-weak-ssl-ciphers no-ssl3 no-ssl3-method no-tls1 no-tls1-method no-tls1_1 no-tls1_1-method no-dtls1 no-dtls1-method mingw64 --prefix=${SYSTEMDRIVE}/Dev/Binaries/OpenSSLx64Release/ --openssldir=${SYSTEMDRIVE}/Dev/Binaries/OpenSSLx64Release/
make --jobs=$(nproc) V=1
...
sleep 8s
rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/*.* ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/* ${SYSTEMDRIVE}/Dev/Builds/OpenSSLx64Release/
echo DONE!
