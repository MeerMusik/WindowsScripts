...
rem %DEVKIT%\bin\zig.exe build -p stage3 --search-prefix %DEVKIT% --zig-lib-dir lib -Drelease -Dstatic-llvm -Duse-zig-libcxx -Dtarget=x86_64-windows-gnu
%SYSTEMDRIVE%\Dev\zig\zig.exe build -p stage3 --search-prefix %DEVKIT% --zig-lib-dir lib -Dstatic-llvm -Duse-zig-libcxx -Dtarget=x86_64-windows-gnu --verbose -Doptimize=ReleaseSafe --prefix %SYSTEMDRIVE%/Dev/Binaries/Zigx64StaticRelease/ -fno-emit-docs -fPIC -fPIE -flto --build-id=sha1
rem %SYSTEMDRIVE%\Dev\zig\zig.exe install
rem cmake %SystemDrive%\Dev\Builds\Zigx64Debug\ -G"Ninja" -DCMAKE_BUILD_TYPE="Debug" -DCMAKE_PREFIX_PATH="%DEVKIT%" -DCMAKE_C_COMPILER="%DEVKIT%/bin/zig.exe;cc" -DCMAKE_CXX_COMPILER="%DEVKIT%/bin/zig.exe;c++" -DCMAKE_AR="%DEVKIT%/bin/zig.exe" -DZIG_AR_WORKAROUND=ON -DZIG_STATIC=ON -DZIG_USE_LLVM_CONFIG=OFF -A x64 && ^
