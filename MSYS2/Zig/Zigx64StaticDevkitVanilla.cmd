...
%SYSTEMDRIVE%\Dev\zig\zig.exe build -p stage3 --search-prefix %DEVKIT% --zig-lib-dir lib -Dstatic-llvm -Duse-zig-libcxx -Dtarget=x86_64-windows-gnu --verbose -Dno-langref -Dno-autodocs -Doptimize=Debug --prefix %SYSTEMDRIVE%/Dev/Binaries/Zigx64StaticDebug/ && ^
...
:PREPAREBUILDRELEASE
set DEVKIT=%SystemDrive%\Dev\zigdevkit\
if not exist %SystemDrive%\Dev\Builds\Zigx64Release\ ( 
mkdir %SystemDrive%\Dev\Builds\Zigx64Release\ 
goto :BUILDRELEASE 
)

if exist %SystemDrive%\Dev\Builds\Zigx64Release\ ( 
cd %SystemDrive%\Dev\Builds\Zigx64Release\
...
del *.* /s /f /q
cd %SystemDrive%\
rmdir %SystemDrive%\Dev\Builds\Zigx64Release\ /s /q
echo Wait for Windows to release the Directory handle, then delete the Directory:
timeout /t 8 /nobreak
mkdir %SystemDrive%\Dev\Builds\Zigx64Release\
goto :BUILDRELEASE 
...
:BUILDRELEASE
cd %SystemDrive%\Dev\Builds\Zigx64Release\ && ^
robocopy /s /NJS /NDL /NJH /nc /ns /np %SystemDrive%\Dev\GitRepos\zig\ %SystemDrive%\Dev\Builds\Zigx64Release\ /xd ".git" & ^
cd %SystemDrive%\Dev\Builds\Zigx64Release\ && ^
rem BUG How to build the Language Reference and Docs??
rem BUG Which is the current correct Default Value for Release Builds? ReleaseFast? ReleaseSafe? ReleaseSmall?
%SYSTEMDRIVE%\Dev\zig\zig.exe build -p stage3 --search-prefix %DEVKIT% --zig-lib-dir lib -Dstatic-llvm -Duse-zig-libcxx -Dtarget=x86_64-windows-gnu --verbose -Dno-langref -Dno-autodocs -Doptimize=ReleaseSmall --prefix %SYSTEMDRIVE%/Dev/Binaries/Zigx64StaticRelease/ && ^
rem BUG How to use Build Tests? Docs are outdated!
rem stage3\bin\zig.exe build test
cd %SystemDrive%\Dev\Builds\Zigx64Release\ && ^
del *.* /s /f /q && ^
