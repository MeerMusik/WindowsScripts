...
if [ -d ${SYSTEMDRIVE}/Dev/Builds/Zigx64/ ]; then rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/Builds/Zigx64/*.* ${SYSTEMDRIVE}/Dev/Builds/Zigx64/* ${SYSTEMDRIVE}/Dev/Builds/Zigx64/
fi
...
cmake . -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX="${SYSTEMDRIVE}/Dev/Binaries/Zigx64Debug/"
make --jobs=$(nproc) V=1
make check V=1
...
cmake . -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX="${SYSTEMDRIVE}/Dev/Binaries/Zigx64Release/"
make --jobs=$(nproc) V=1
make check V=1
echo Script paused for 10 Seconds to be able to read the Test Results! Please wait a Moment...
sleep 10s
