...
if [ -d ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/ ]; then rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/*.* ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/* ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/
fi
if [ ! -d ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/ ]; then mkdir ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/
fi
cd ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/
cmake ${SYSTEMDRIVE}/Dev/GitRepos/llvm-project/llvm/ ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/ -G'Ninja' -DCMAKE_VERBOSE_MAKEFILE=ON -DCMAKE_C_FLAGS_RELEASE='-fasynchronous-unwind-tables -fexceptions -finput-charset=utf-8 -fno-lto -fno-omit-frame-pointer -grecord-gcc-switches -m64 -pthread -O3 -g0 -pipe -Wall -Wl,-O3 -Wl,--allow-multiple-definition -Wl,--build-id=sha1 -Wl,--dynamicbase -Wl,--enable-auto-image-base -Wl,-fno-lto -Wl,--high-entropy-va -Wl,--nxcompat -Wa,-mbig-obj' -DCMAKE_CXX_FLAGS_RELEASE='-fasynchronous-unwind-tables -fexceptions -finput-charset=utf-8 -fno-lto -fno-omit-frame-pointer -grecord-gcc-switches -m64 -pthread -O3 -g0 -pipe -Wall -Wl,-O3 -Wl,--allow-multiple-definition -Wl,--build-id=sha1 -Wl,--dynamicbase -Wl,--enable-auto-image-base -Wl,-fno-lto -Wl,--high-entropy-va -Wl,--nxcompat -Wa,-mbig-obj' -DLLVM_ENABLE_PROJECTS='clang;clang-tools-extra;compiler-rt;lld;lldb;llvm' -DCMAKE_CXX_STANDARD=17 -DCMAKE_INSTALL_PREFIX=${SYSTEMDRIVE}/Dev/Binaries/LLVMx64Release/ -DCMAKE_BUILD_TYPE='Release' -DLLVM_TARGETS_TO_BUILD='X86' -DLLVM_INCLUDE_EXAMPLES='OFF' -DLLVM_INCLUDE_TESTS='OFF' -DLLVM_INCLUDE_DOCS='OFF' -DLLVM_PARALLEL_COMPILE_JOBS=$(nproc) -DLLVM_PARALLEL_LINK_JOBS=$(nproc) -DLLVM_ENABLE_EH='ON' -DLLVM_ENABLE_RTTI='ON' -DLLVM_ENABLE_DIA_SDK='OFF' -DLLVM_ENABLE_ASSERTIONS='OFF'
ninja
echo Script paused for 10 Seconds to be able to read the Test / Build Results! Please wait a Moment...
sleep 10s
ninja install
rm --interactive=never --dir --recursive --force --verbose ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/*.* ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/* ${SYSTEMDRIVE}/Dev/Builds/LLVMx64Release/
exit
